<template>
	...
	<view class="post">
		<view v-if="!post.data.isAnonymous" class="user">
			...
			<view v-for="(item, index) in comments.data" :key="index" class="comment-box">
				...
				<view class="like-box">
					<view v-if="comments.likeData[index]">
						<image :src="comments.likeData[index].likeUrl" class="like-icon" mode="widthFix"
	...
	<view class="write-comment-box">
		<input v-model="text" class="write-comment" placeholder="发表评论..." type="text" />
		<view class="like-box">
			<image :src="postLike.likeUrl" class="like-icon" mode="widthFix" @click="postDoLike()" />
			<view class="like-num">
				{{ postLike.count }}
			</view>
		</view>
