	...
	<view class="page">
		<view class="search-bar">
			<view class="search-bar-box">
				<image class="ml-2" style="width: 40rpx; margin-top: 17rpx" mode="widthFix"
					src="/static/images/search.png" />
				<input ref="getValue" v-model="searchText" class="search-text" maxlength="20" placeholder="搜索帖子|动态|图鉴"
					type="text" />
			</view>
		...
		<view v-if="!isClickSearch" class="">
			...
			<view v-if="list.length !== 0" class="px-1 mb-2">
				<view v-for="(item, index) in list" :key="index" hover-class="bg-light"
					class="px-2 py-1 border d-inline-block m-1 font-md" :style="getStyle" style="border-radius: 20rpx"
		...
		<view v-if="isClickSearch" class="">
			<view style="margin-top: 20upx">
				<zzx-tabs ref="mytabs" :items="items" :current="current" @click-item="onClickItem">
				</zzx-tabs>
			</view>
			<view style="
          margin-top: 20upx;
...
<script setup lang="ts">
	...
	onLoad(() => {
		...
		if (list.value.length === 0 && historyText) {
			list.value = JSON.parse(decodeURIComponent(uni.getStorageSync("historySearchText")));
		}
	});
	...
	function onClickSearch() {
		...
		if (searchText.value !== "") {
			...
			if (list.value.length !== 0) {
				uni.setStorageSync("historySearchText", encodeURIComponent(JSON.stringify(list.value)));
			}
		} else {
			uni.showToast({
				title: "请输入搜索内容",
