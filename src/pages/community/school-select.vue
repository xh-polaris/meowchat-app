<template>
	<view class="content2">
		<view class="d-flex border a-center mx-2 j-sb px-3"
			style="border-radius: 70rpx;height: 60rpx;border-color: #938B8E;border-width: 4rpx;">
			<input class="search-text" maxlength="10" placeholder="输入学校名称或拼音查询" type="text" value="" />
			<image style="width: 40rpx;" mode="widthFix" src="/static/images/search.png" />
		</view>
		...
		<view class="school-select-box">
			...
			<view v-else class="box">
				...
				<view class="option" @click="change">
					<text v-for="(item, index) in school.campuses" :key="index" :src="item" @click="changeCampus(item)">
						{{ item }}
					</text>
				</view>
			</view>
		...
		<view class="big">
			<view>
				<view v-for="(item, index) in school.name" :key="index" :src="item" class="bubble"
	...
	<view class="content2">
		<view v-for="(item, index) in school.alpha" :key="index" class="school-bar" :src="item">
			<view class="small"> {{ item }}</view>
			<view v-for="(item1, index1) in school.name" :key="index1" :src="school" class="school"
